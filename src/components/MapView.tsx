...
// Compute bounding box for Polygon or MultiPolygon
...
export const MapView = () => {
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      map.current.addLayer({
        ...
        paint: { 'fill-color': ['match', ['get', 'scope_status'], 'Active', '#003f5c', 'Upcoming', '#2f4b7c', 'Under discussion', '#ffa600', '#cccccc'], 'fill-opacity': ['case', ['boolean', ['feature-state', 'hover'], false], 0.7, 0.3] }
      });
      map.current.addLayer({
        id: 'schemes-outline',
        type: 'line',
      ...
      map.current.addLayer({
        ...
        paint: { 'circle-color': ['match', ['get', 'scope_status'], 'Active', '#003f5c', 'Upcoming', '#2f4b7c', 'Under discussion', '#ffa600', '#cccccc'], 'circle-radius': ['interpolate', ['linear'], ['zoom'], 2, 6, 10, 10, 15, 14], 'circle-stroke-width': 2, 'circle-stroke-color': '#fff' }
      });
      ...
      map.current.on('mouseenter', 'schemes-icons', (e) => {
        ...
        if (id) { map.current!.setFeatureState({ source: 'schemes-polygons', id }, { hover: true }); map.current!.setFeatureState({ source: 'schemes-icons', id }, { hover: true }); }
      });
      map.current.on('mouseleave', 'schemes-icons', () => {
        map.current!.getCanvas().style.cursor = '';
        if (hoveredSchemeId) { map.current!.setFeatureState({ source: 'schemes-polygons', id: hoveredSchemeId }, { hover: false }); map.current!.setFeatureState({ source: 'schemes-icons', id: hoveredSchemeId }, { hover: false }); }
  ...
  useEffect(() => {
    ...
    // Fit map to first selected region
    ...
    const polygonFeatures = sorted.map(s => ({ type: 'Feature', id: s.id, properties: { id: s.id, name: s.regulation_name, scope_status: s.scope_status, scope_region: s.scope_region }, geometry: ensurePolygonGeometry(s.geometry) }));
    const iconFeatures = sorted.map(s => { const pos = s.icon_position || calculateIconPosition(s.geometry); return { type: 'Feature', id: s.id, properties: { id: s.id, name: s.regulation_name, scope_status: s.scope_status }, geometry: { type: 'Point', coordinates: [pos.lng, pos.lat] } }; });
    ...
    if (polySource && iconSource) { polySource.setData({ type: 'FeatureCollection', features: polygonFeatures }); iconSource.setData({ type: 'FeatureCollection', features: iconFeatures }); }
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {tooltipSchemes && tooltipPosition && (
        <SchemeTooltip schemes={tooltipSchemes} position={tooltipPosition} onViewDetails={selectScheme} onClose={() => { setTooltipSchemes(null); setTooltipPosition(null); }} />
      )}
    </div>
  );
};
